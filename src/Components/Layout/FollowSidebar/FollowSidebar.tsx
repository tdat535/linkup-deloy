...
const FollowSidebar: React.FC<FollowSidebarProps> = ({ onReady }) => {
    ...
    const fetchFollowings = async () => {
      try {
        ...
        const res = await axiosInstance.get(
          "https://api-linkup.id.vn/api/follow/getFollow",
          {
            headers: {
  ...
  if (isMobile) {
    return (
      <header
        ...
        } flex items-center p-4 border-b border-gray-300`}
      >
        ...
        <div className="relative">
          ...
          {isMenuOpen && (
            <div
              ...
              } rounded-lg shadow-lg text-sm p-2`}
            >
  ...
  return (
    <aside
      className={`fixed top-0 right-0 h-full w-64 p-4 border-l border-gray-300 ${
        theme === "dark" ? "bg-[#1C1C1D] text-white" : "bg-[#f0f2f5] text-black"
      }`}
    >
      ...
      <ul>
        {followings.map((user) => (
          <li
            ...
            className="flex items-center gap-3 mb-3 hover:bg-gray-700 p-2 rounded-lg cursor-pointer transition duration-200"
          >
            <img
              src={user.avatar}
              alt={user.username}
              className="rounded-full w-9 h-9 border border-gray-400"
            />
